...
[n0][n1][n2][n3][n4][n5][n6][n7]amix=inputs=8:normalize=0:duration=longest,apad=whole_dur=59[vo];\
[vo]asplit=2[vomix][vosc];\
[ac]volume=0.5[bedlo];[bedlo][vosc]sidechaincompress=threshold=0.035:ratio=9:attack=15:release=420[bed];\
[bed][vomix]amix=inputs=2:normalize=0:duration=first,loudnorm=I=-14:TP=-1.5:LRA=11,afade=t=out:st=57.0:d=1.4[aout]" \
 -map "[vout]" -map "[aout]" -c:v libx264 -b:v 2900k -maxrate 3400k -bufsize 6000k -preset medium -t 58 -pix_fmt yuv420p -r 24 \
 -c:a aac -b:a 192k -ar 48000 -movflags +faststart QDAY_EP0_act3_v3_fixed.mp4 -y
