...
[n0][n1][n2][n3][n4][n5][n6][n7]amix=inputs=8:normalize=0:duration=longest,volume=2.5,apad=whole_dur=92[vo];\
[vo]asplit=2[vomix][vosc];\
[ac][vosc]sidechaincompress=threshold=0.035:ratio=9:attack=15:release=420[bed];\
[bed][vomix]amix=inputs=2:normalize=0:duration=first,alimiter=limit=0.95,loudnorm=I=-14:TP=-1.5:LRA=11,afade=t=out:st=89.0:d=1.6[aout]" \
 -map "[vout]" -map "[aout]" -c:v libx264 -b:v 2900k -maxrate 3400k -bufsize 6000k -preset medium -t 90 -pix_fmt yuv420p -r 24 \
 -c:a aac -b:a 192k -ar 48000 -movflags +faststart QDAY_EP0_act1.mp4 -y
