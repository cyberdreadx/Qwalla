...
[15:a]aformat=sample_fmts=fltp:sample_rates=48000:channel_layouts=stereo,adelay=19040|19040[n1];\
...
[n0][n1][n2][n3][n4]amix=inputs=5:normalize=0:duration=longest,apad=whole_dur=62[vo];\
[vo]asplit=2[vomix][vosc];\
[ac]volume=0.5[bedlo];[bedlo][vosc]sidechaincompress=threshold=0.035:ratio=9:attack=15:release=420[bed];\
[bed][vomix]amix=inputs=2:normalize=0:duration=first,loudnorm=I=-14:TP=-1.5:LRA=11,afade=t=out:st=57.6:d=1.4[aout]" \
 -map "[vout]" -map "[aout]" -c:v libx264 -b:v 2900k -maxrate 3400k -bufsize 6000k -preset medium -t 59 -pix_fmt yuv420p -r 24 \
 -c:a aac -b:a 192k -ar 48000 -movflags +faststart QDAY_EP0_teaser_v2.mp4 -y
